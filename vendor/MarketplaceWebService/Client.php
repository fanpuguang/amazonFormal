 ...
 //    iconv_set_encoding('internal_encoding', 'UTF-8');
  ...
  public function submitFeed($request)
  {
    ...
    if (!$request instanceof MarketplaceWebService_Model_SubmitFeedRequest) {
      require_once (dirname(dirname(__FILE__)).'/MarketplaceWebService/Model/SubmitFeedRequest.php');
      $request = new MarketplaceWebService_Model_SubmitFeedRequest($request);
    }
    require_once (dirname(dirname(__FILE__)).'/MarketplaceWebService/Model/SubmitFeedResponse.php');
 ...
 // dump($httpResponse['ResponseBody']);
  ...
  public function GetFeedSubmissionResult($request)
  {
    if (!$request instanceof MarketplaceWebService_Model_GetFeedSubmissionResultRequest) {
        require_once (dirname(dirname(__FILE__)).'/MarketplaceWebService/Model/GetFeedSubmissionResultRequest.php');
      $request = new MarketplaceWebService_Model_GetFeedSubmissionResultRequest($request);

    }
      require_once (dirname(dirname(__FILE__)).'/MarketplaceWebService/Model/GetFeedSubmissionResultResponse.php');
    $httpResponse = $this->invoke($this->convertGetFeedSubmissionResult($request), $request->getFeedSubmissionResult());
    return $httpResponse;
      // dump($httpResponse);die;
  ...
  public function getFeedSubmissionList($request)
  {
    ...
    if (!$request instanceof MarketplaceWebService_Model_GetFeedSubmissionListRequest) {
      require_once ('/www\wwwroot\www.ldkjmy.com\aaaa\yamaxun\vendor\MarketplaceWebService/Model/GetFeedSubmissionListRequest.php');
    ...
    require_once ('/www/wwwroot/www.ldkjmy.com/aaaa/yamaxun/vendor/MarketplaceWebService/Model/GetFeedSubmissionListResponse.php');
    ...
    return $httpResponse['ResponseBody'];
    // dump($httpResponse['ResponseBody']);die;
  ...
  private function invoke(array $converted, $dataHandle = null)
  {
    ...
    if (empty($this->config['ServiceURL'])) {
      ...
      do {
        try {
          ...
          switch ($httpStatus) {
          	...
          	case 500:
          	case 503:
          		require_once(dirname(dirname(__FILE__)).'/MarketplaceWebService/Model/ErrorResponse.php');
          ...
          require_once(dirname(dirname(__FILE__)).'/MarketplaceWebService/Exception.php');
            dump($e);die;
      ...
      vendor("MarketplaceWebService.Exception");
...
//    dump(dirname(__FILE__).'/Exception.php');die;
  ...
  private function performRequest($action, array $converted, $dataHandle = null) {
    ...
    curl_setopt($this->curlClient, CURLOPT_SSL_VERIFYPEER, false); //不验证证书下同
