...
class Sys extends Base
{
    ...
    public function sys()
    {
        $ean_list=Db::name('ean')->where('uid ='.session('admin_auth.aid'))->order('id desc')->paginate(config('paginate.list_rows'),false,['query'=>get_query()]);
        $show = $ean_list->render();
        $show=preg_replace("(<a[^>]*page[=|/](\d+).+?>(.+?)<\/a>)","<a href='javascript:ajax_page($1);'>$2</a>",$show);
        $this->assign('page',$show);
        $this->assign('ean_list',$ean_list);
        if(request()->isAjax()){
            return $this->fetch('ajax_eansetsys');
        }else{
            return $this->fetch();
        }
    }
    ...
    {
        if (!request()->isAjax()){
            $this->error('提交方式不正确',url('admin/Sys/eansetsys'));
        }

        //当前登陆账号id
        $uid = session('admin_auth.aid');
        $data['uid'] = $uid;
        $data['bmlx'] = input('bmlx');
        $data['cjbh'] = input('cjbh');
        $data['cpbh'] = input('cpbh');
        $data['scsl'] = input('scsl');
        $data['add_time'] = time();
        //判断输入的数据是否合法
        if(strlen($data['cjbh'])!= 4){
            $this->error('抱歉厂家编号数据不合法,请重新输入!');
            return false;
        }
        if(strlen($data['cpbh'])!= 5){
            $this->error('抱歉产品编号数据不合法,请重新输入!');
            return false;
        }
        if(strlen($data['scsl'])>3){
            $this->error('抱歉生产数量超出限制,请重新输入!');
            return false;
        }
        //修改用户上次记录表
        $result =  Db::name('ean_jl')->insert($data);
        $p=input('p',1,'intval');
        if (!$result){
            $this->error ('生成失败，请修正后重新生成',url('admin/Sys/sys',array('p'=>$p)),1);
        }else{
            for ($x=0; $x<=$data['scsl']; $x++) {

                $ean = $this->ean_jc($data['cjbh'],$data['cpbh']);
                $data1['uid'] = $uid;
                $data1['bianma'] = $ean;
                $data1['type'] =input('bmlx');;
                //生成ean码数据
                Db::name('ean')->insert($data1);
            }

            $this->success('EAN生成完毕',url('admin/Sys/sys',array('p'=>$p)),1);
        }
	 ...
	 * @author liujie
     */
    public function bianma_del()
    {
        $rst=Db::name('ean')->where(array('id'=>input('id')))->delete();
        if($rst!==false){
            $p=input('p',1,'intval');
            $this->success('编码删除成功',url('admin/Sys/sys',array('p'=>$p)));
        }else{
            $this->error('编码删除失败',url('admin/Sys/sys'));
        }
    }
    /*
    * 清空EAN/UPC码
    * @author liujie
    */
    public function bianma_dels()
    {
        $rst=Db::name('ean')->where(array('uid'=>session('admin_auth.aid')))->delete();
        if($rst!==false){
            $p=input('p',1,'intval');
            $this->success('编码清空成功',url('admin/Sys/sys',array('p'=>$p)));
        }else{
            $this->error('编码清空失败',url('admin/Sys/sys'));
        }
    }
    ...
    public function excel_runean()
    {
        ...
        if (! empty ( $_FILES ['file_stu'] ['name'] )){
            ...
            if (strtolower ( $file_type ) != "xlsx"){
                $this->error ( '不是Excel文件，重新上传',url('admin/Sys/sys'));
            }
            /*设置上传路径*/
            $savePath =ROOT_PATH. 'public/excel/';
            /*以时间来命名上传的文件*/
            ...
            $file_name = $str . "." . $file_type;
            if (! copy ( $tmp_file, $savePath . $file_name )){
                $this->error ('上传失败',url('admin/Sys/sys'));
            }

            $res = read ( $savePath . $file_name );

            if (!$res){
                $this->error ('数据处理失败',url('admin/Sys/sys'));
            }
            ...
            foreach ( $res as $k => $v ){
                ...
                if ($k != 1){
                    $data['bianma']=$v[0];
                    $data['uid']=$uid;
                    $result = Db::name ('ean')->insert($data);
                    if (!$result){
                        $this->error ('导入编码失败',url('admin/Sys/sys'));
                    }
                }
            }
            $this->success ('导入编码成功',url('admin/Sys/sys'));
        }
    }
    //生成随机EAN码
    public function ean_jc($csbm,$sobm){
        //随机前三位中国
        $qsw = rand(690, 695);
        //效验码
        $xy = rand(1, 9);

        $csbm = str_shuffle($csbm);//打乱字符串
        $sobm = str_shuffle($sobm);//打乱字符串
        //EAN码
        $ean = $qsw.$csbm.$sobm.$xy;
        $N2 = substr($ean , 11 , 1);
        $N3 = substr($ean , 10 , 1);
        $N4 = substr($ean , 9 , 1);
        $N5 = substr($ean , 8 , 1);
        ...
        $C1 = ($N2+$N4+$N6+$N8+$N10+$N12)*3;
        $C2 =$N3+$N5+$N7+$N9+$N11+$N13;
        $C4 =$C1+$C2;
        $C3 = substr($C4, -1);
        $N1 = (10-$C3);
        $EAN= $N13.$N12.$N11.$N10.$N9.$N8.$N7.$N6.$N5.$N4.$N3.$N2.substr($N1, -1);
        return $EAN;
    }
