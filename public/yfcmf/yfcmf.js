...
/* 服务器编辑 */
$(function () {
    $('body').on('click','.fwqedit-btn',function () {
        var $url = this.href,
            val = $(this).data('id');
        $.post($url, {fwq_id: val}, function (data) {
            if (data.code == 1) {
                $("#myModaledit").show(300);
                $("#editfwq_id").val(data.fwq_id);
                $("#edit_fwq_ip").val(data.fwq_ip);
                $("#edit_fwq_name").val(data.fwq_name);
                $("#edit_fwq_password").val(data.fwq_password);
                $("#edit_gm_time").val(data.gm_time);
                $("#edit_dq_time").val(data.dq_time);
                $("#edit_yyzzfr").val(data.yyzzfr);
                $("#edit_fwq_gys").val(data.fwq_gys);
                $("#edit_fwq_glz").val(data.fwq_glz);
                $("#edit_fwq_dyctzh").val(data.fwq_dyctzh);
            } else {
                layer.alert(data.msg, {icon: 5});
            }
        }, "json");
...
/* 账户信息z */
$(function () {
    $('body').on('click','.zhedit-btn',function () {
        var $url = this.href,
            val = $(this).data('id');
            $("#yonghu_id").val(val);

        $("#myModaledit").show(300);
        $.post($url, {admin_id: val}, function (data) {
            if (data.code == 1) {
...
/* 账号授权编辑 */
$(function () {
    $('body').on('click','.sqedit-btn',function () {
        var $url = this.href,
            val = $(this).data('id');
        ...
        $.post($url, {plug_sug_id: val}, function (data) {
            if (data.code == 1) {
                ...
                $("#myModaledit").show(300);
                $("#edit_plug_sug_id").val(data.plug_sug_id);
                $("#edit_plug_dpbm").val(data.plug_dpbm);
                $("#edit_plug_sug_name").val(data.plug_sug_name);
                $("#edit_plug_merchant_id").val(data.plug_merchant_id);
                $("#edit_plug_lingpai").val(data.plug_lingpai);
                $("#edit_plug_khqy").val(data.plug_khqy);
...
/* 产品分类编辑 */
$(function () {
    $('body').on('click','.productclassedit-btn',function () {
        var $url = this.href,
            val = $(this).data('id');
        $.post($url, {id: val}, function (data) {
            if (data.code == 1) {
