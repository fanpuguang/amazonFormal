...
{
    $adminname= Db::name('admin')->where('admin_id='.$uid)->field('admin_realname')->find();

    return $adminname['admin_realname'];
}
 ...
 * @author rainfer <81818832@qq.com>
 */
function read($filename,$type='Excel2007')
{
...
{
    $aa = new Database();
    $list_fenlei =$aa->db2->table('ProductCategories')->where('categoryId ='.$class)->find();

    return $list_fenlei['categoryName'];
}
/**
 * 根据用户id获取用户授权账号昵称
 * @param   int $uid    用户id
 * @return string
 */
function get_shouquanname($uid)
{
    $plug_dpbm= Db::name('plug_sug')->where('plug_uid',$uid)->field('plug_dpbm')->find();

    return $plug_dpbm['plug_dpbm'];
}
 ...
 * @return string
 */
function get_dingjiname($uid)
{
    $adminname= Db::name('admin')->where('admin_id='.$uid)->field('fj_id')->find();
    if(empty($adminname['fj_id'])){
        return '';
    }else{
        $adminnames= Db::name('admin')->where('admin_id='.$adminname['fj_id'])->field('admin_realname')->find();
        return $adminnames['admin_realname'];
    }
