...
// +----------------------------------------------------------------------
// | Author: rainfer <337064734@qq.com>
// +----------------------------------------------------------------------
...
class plug extends Base
{
	...
	public function plug_sug_list()
	{
		...
		$plug_sug=Db::name('plug_sug')->order('plug_sug_addtime desc')->where('plug_uid = '.$uid)->paginate(config('paginate.list_rows'));
	...
	public function plug_sug_runreply()
	{
        ...
        if (!request()->isAjax()){
            $this->error('提交方式不正确',url('admin/plug/plug_sug_list'));
        }else{
            $uid = session('admin_auth.aid');

            $sl_data=array(
                'plug_dpbm'=>input('plug_dpbm'),
                'plug_sug_name'=>input('plug_sug_name'),
                'plug_khqy'=>$_POST['plug_khqy'],
                'plug_merchant_id'=>input('plug_merchant_id'),
                'plug_lingpai'=>input('plug_lingpai'),
                'plug_uid'=>input('plug_link_uid',$uid),
                'plug_sug_addtime'=>time(),
            );
            $rst=Db::name('plug_sug')->insert($sl_data);
            if($rst!==false){
                $this->success('平台授权添加成功',url('admin/plug/plug_sug_list'));
            }else{
                $this->error('平台授权添加失败',url('admin/plug/plug_sug_list'));
            }
        }
	}
    ...
    public function plug_sug_edit()
    {
        ...
        $fwq=Db::name('plug_sug')->where(array('plug_sug_id'=>$plug_sug_id))->find();
        $sl_data['plug_sug_id']=$fwq['plug_sug_id'];
        $sl_data['plug_dpbm']=$fwq['plug_dpbm'];
        $sl_data['plug_sug_name']=$fwq['plug_sug_name'];
        $sl_data['plug_khqy']=$fwq['plug_khqy'];
        $sl_data['plug_merchant_id']=$fwq['plug_merchant_id'];
        $sl_data['plug_lingpai']=$fwq['plug_lingpai'];
        $sl_data['code']=1;
     ...
     * @author liujie
     */
    public function plug_sug_runedit()
    {
        if (!request()->isAjax()){
            $this->error('提交方式不正确',url('admin/Plug/plug_sug_list'));
        }else{
            ...
            $rst=Db::name('plug_sug')->update($sl_data);
            if($rst!==false){
                $this->success('店铺授权修改成功',url('admin/Plug/plug_sug_list'));
            }else{
                $this->error('店铺授权修改失败',url('admin/Plug/plug_sug_list'));
            }
        }
    }
     ...
     * @author rainfer <337064734@qq.com>
     */
    public function plug_sug_runreply11()
    {
        ...
        if($send_result['error']){
            ...
            if($rst!==false){
                $this->success('回复亚马逊授权成功',url('admin/plug/plug_sug_list'));
            }else{
                $this->error('回复亚马逊授权失败',url('admin/plug/plug_sug_list'));
            }
        }
    }
    /*
    ...
    public function plug_sug_reply()
    {
        ...
        $rule = [
            ['plug_sug_email','require|email','亚马逊授权邮箱为空|亚马逊授权邮箱格式不正确'],
        ];
	...
	public function plug_sug_del()
	{
		...
		if($rst!==false){
			$this->success('亚马逊授权删除成功',url('admin/plug/plug_sug_list',array('p'=>$p)));
		}else{
			$this->error('亚马逊授权删除失败',url('admin/plug/plug_sug_list',array('p'=>$p)));
		}
	}
	/*
     * 亚马逊授权删除(全选)
	...
	public function plug_sug_alldel()
	{
		...
		$ids = input('sug_id/a');
		if(empty($ids)){
			$this -> error("请选择删除亚马逊授权",url('admin/plug/plug_sug_list',array('p'=>$p)));
		}
		if(is_array($ids)){
		...
		if($rst!==false){
			...
			$this -> error("删除亚马逊授权失败！",url('admin/plug/plug_sug_list',array('p' => $p)));
		}
	}
	/*
     * 友情链接列表
