...
class Admin extends Model
{
     ...
     * 增加管理员
     */
    public static function add($admin_username,$admin_pwd_salt='',$admin_pwd,$admin_email='',$admin_tel='',$admin_open=0,$admin_realname='',$en_name='',$en_brand='',$group_id='',$fj_id='')
    {
        ...
        $sldata=array(
           ...
           'member_id'=>session('admin_auth.aid'),
            'fj_id'=>$fj_id,
            'admin_tel'=>$admin_tel,
        ...
        //查询账号是否存在
        $map['admin_username'] =$admin_username;
       $zhanghao  = Db::name('admin')->where( $map)->count();
        if($zhanghao > 0 ){
            return 0;
        }
     ...
     * 分账号增加管理员
     */
    public static function add1($admin_username,$admin_pwd_salt='',$admin_pwd,$admin_email='',$admin_tel='',$admin_open=0,$admin_realname='',$en_name='',$en_brand='',$group_id='',$fj_id='')
    {
        $admin_pwd_salt=$admin_pwd_salt?:random(10);
        $sldata=array(
            'admin_username'=>$admin_username,
    ...
    public static function edit($data)
    {
        ...
        $admin['en_name']=$data['en_name'];
        $admin['en_brand']=$data['en_brand'];
        //$admin['money']=$data['money'];
        //dump($admin['fj_id']);die;
        /*if($admin['fj_id'] == 0){
            $admin['fj_id']=$data['fj_id'];
        }*/
    ...
    public static function edit1($data)
    {
        ...
        $admin=self::get($data['admin_id'])->toArray();
        $data['admin_username'] = $admin['admin_username'];
        $admin['admin_email']=$data['admin_email'];
        $admin['admin_tel']=$data['admin_tel'];
        $admin['admin_realname']=$data['admin_realname'];
        $data['admin_open']=$admin['admin_open'];
        $admin['en_name']=$data['en_name'];
        $admin['en_brand']=$data['en_brand'];
       //
        if($data['admin_pwd']){
        ...
        //判断登陆的账号是否是该公司的总账号
        $admin1=Db::name('admin')->where(array('admin_id'=>$data['admin_id']))->field('fj_id')->find();
        if($admin1['fj_id'] == session('admin_auth.aid')){
            $rst=self::where('admin_id',$data['admin_id'])->update($admin);
        }else{
            return false;
        }
     ...
     * @return bool
     */
    public static function edit2($data)
    {
        $data['admin_id'] = session('admin_auth.aid');
        $admin['admin_email']=$data['admin_email'];
        $admin['admin_tel']=$data['admin_tel'];
