...
//                        require_once (dirname(dirname(__FILE__))."\\".str_replace('_', DIRECTORY_SEPARATOR, $fieldType[0]) . ".php");
                        require_once (dirname(dirname(__FILE__))."/".str_replace('_', DIRECTORY_SEPARATOR, $fieldType[0]) . ".php");
                ...
                if ($this->isComplexType($fieldType)) {
                    $elements = $xpath->query("./a:$fieldName", $dom);
                    if ($elements->length == 1) {
                       // echo dirname(dirname(__FILE__))."/".str_replace('_', DIRECTORY_SEPARATOR, $fieldType) . ".php";die;
                        require_once (dirname(dirname(__FILE__))."/".str_replace('_', DIRECTORY_SEPARATOR, $fieldType) . ".php");
    ...
    private function fromAssociativeArray(array $array)
    {
        foreach ($this->fields as $fieldName => $field) {
            ...
            if (is_array($fieldType)) {
                ...
                if ($this->isComplexType($fieldType)) {
                    if (array_key_exists($fieldName, $array)) {
                        // dump(str_replace('_', DIRECTORY_SEPARATOR, $fieldType));
                        // dump("D:/phpStudy/PHPTutorial/WWW/tp5/vendor/");
                        require_once (dirname(dirname(__FILE__))."/".str_replace('_', DIRECTORY_SEPARATOR, $fieldType) . ".php");
