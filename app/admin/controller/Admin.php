...
class Admin extends Base
{
	...
	public function admin_list()
	{
		...
		$admin_list=Db::name('admin')->where($map)->order('fj_id,admin_id')->paginate(config('paginate.list_rows'),false,['query'=>get_query()]);
	...
	public function admin_add()
	{
        ...
        //公司用户组
        $gongsi_group=Db::name('admin')->where('fj_id = 1')->field('admin_id,admin_realname')->select();
        $this->assign('gongsi_group',$gongsi_group);
		return $this->fetch();
	}
	/**
	 * 管理员添加操作
	...
	public function admin_runadd()
	{
       ...
       if($fj_id == ''){
		...
		$admin_id=AdminModel::add(input('admin_username'),'',input('admin_pwd'),input('admin_email',''),input('admin_tel',''),input('admin_open',0),input('admin_realname',''),input('en_name',''),input('en_brand',''),input('group_id'),$fj_id);
		if($admin_id){
     ...
     * 个人信息修改操作
     */
    public function admin_runedit1()
    {
        $data=input('post.');
        ...
        if($rst!==false){
            ...
            $this->error('管理员修改失败',url('admin/Admin/profile'));
        }
    }
    /**
     * 个人信息修改操作
     */
    public function admin_runedit2()
    {
        $data=input('post.');
//dump($data);die;
        $rst=AdminModel::edit2($data);
        if($rst!==false){
	...
	public function profile()
	{
		...
		if(session('admin_auth.aid')){
			...
			$product_count=Db::name('Goods')->where(array('uid'=>session('admin_auth.aid')))->count();
			$admin['product_count']=$product_count;
		}
    ...
    {
        if (!request()->isAjax()){
            $this->error('提交方式不正确',url('admin/Admin/admin_list'));
        }else{
            $admin_id = input('yonghu_id');
            $money = input('moneys','');
            $admin=Db::name('admin')->where(array('admin_id'=>$admin_id))->field('money')->find();
            /*开启事务*/
            Db::startTrans();
            try{
                //执行分账号修改金额
                ...
                //添加充值记录表
                $data['add_time']=time();
                $data['cz_uid']=input('yonghu_id');
                $data['money']=input('moneys');
                Db::name('money_czjl')->insert($data);
                Db::commit();

            }catch (\Exception $e){
                Db::rollback();
            }
            $this->success('账号充值成功',url('admin/Admin/admin_list'));
